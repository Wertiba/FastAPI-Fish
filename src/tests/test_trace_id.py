...
def test_trace_id_header_is_echoed_back_when_provided(client):
    response = client.get("/api/v1/health/liveness", headers={"X-Trace-Id": "fixed-trace-1"})
...
def test_cors_allows_configured_origin(client):
    response = client.get("/api/v1/health/liveness", headers={"Origin": "http://localhost"})
    assert response.headers.get("access-control-allow-origin") == "http://localhost"


def test_cors_rejects_unconfigured_origin(client):
    response = client.get("/api/v1/health/liveness", headers={"Origin": "http://evil.example"})
